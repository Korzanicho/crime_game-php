        ...
        //KUPNO CZAPKI WPIERDOLKI
            if(isset($_POST['czapka'])){
                ...
                if($cena>$_SESSION['hajs']){
                    echo "<p class='lose'>Kogo chcesz oszukać, masz za mało hajsu!</p>";
                }
                else{
					if($_SESSION['czapkawpierdolka']==1){
						echo "<p class='lose'>Masz już ten przedmiot!</p>";
					}
					else{
						$_SESSION['hajs']-=$cena;
						$cena=$_SESSION['hajs'];
						...
						echo "<p class='success'>Kupiłeś czapkę wpierdolkę!</p>";
	...
	if(isset($_POST['kolczan'])){
		...
		if($cena>$_SESSION['hajs']){
			...
			if($_SESSION['kolczanprawilnosci']==1){
				...
				echo "<p class='success'>Kupiłeś kołczan prawilności!</p>";
